...
function SphereCollisions({
  ...
  this.getShpereDistances = () => {
    ...
    return { centerDist: minCenterDist, colDir }
  }

  this.startPhysics = (
    cb = () => {},
    { dampingFactor, gravity, freq } = {}
  ) => {
    let lastTime = performance.now()
    if (!freq) freq = 1000 / 60
    if (!dampingFactor) dampingFactor = 1
    if (!gravity) gravity = 1
    const checkRate = 1000 / freq
    setInterval(() => {
      ...
      this.vel[1] -= gravity / checkRate
      this.pos[0] += this.vel[0] * delta
      this.pos[1] += this.vel[1] * delta
      this.pos[2] += this.vel[2] * delta

      const { centerDist, colDir } = this.getShpereDistances()
      this.colliding = centerDist < this.radius
      this.colDir = colDir !== null ? [colDir.x, colDir.y, colDir.z] : [0, 0, 0]

      if (this.colliding && colDir !== null) {
        ...
        const reflectedVel = vel
          .copy()
          .sub(colDir.copy().mult(2 * vel.dot(colDir)))
          .mult(dampingFactor)
        const bounce = moveDist - colDist

        const finalPos = colPos
          .copy()
          .add(reflectedVel.copy().normalize().mult(bounce).mult(dampingFactor))

        if (colDir.y > 0.999) {
          finalPos.set([this.pos[0], prevPos.y, this.pos[2]])
          const stableVel = vel.copy()
          stableVel.y = 0
          reflectedVel.set(stableVel)
        }

        this.vel[0] = reflectedVel.x
        this.vel[1] = reflectedVel.y
        this.vel[2] = reflectedVel.z
      ...
      cb({
        delta,
        colDist: this.radius - centerDist, // sdf approach
      })
    }, freq)
  }
}
