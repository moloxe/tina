...
vec3 applyShadowlessLight(
  ...
  PointLight pointLight = PointLight(ro, vec3(1.), 1., false);
  vec3 diffuseColor = material.color;
  float shininess = material.shininess;
  ...
  vec3 colorLinear = /* ambientColor + */
    diffuseColor *    lambertian * pointLight.color * pointLight.power +
    /* specColor * */ specular   * pointLight.color * pointLight.power;
...
LightHits lightHits(vec3 lightOrigin, vec3 target, int targetIndex) {
...
SceneLightning calcSceneLightning(vec3 ro, vec3 rd) {
  ...
  for(int i = 0; i < pointLights.length(); i++) {
    ...
    if(pl.computeShadows) {
      LightHits lh = lightHits(pl.pos, pos, rm.materialIndex);
